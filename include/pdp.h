...
/* Build a heap-allocated ACL tree; free it with pdp_free_tree(). */
PdpNode *pdp_build_tree(const EdocHeader *header);

/* Free a tree returned by pdp_build_tree(). */
void pdp_free_tree(PdpNode *root);

/* Pretty-print the ACL tree; call with depth = 0 for the root. */
void pdp_print_tree(const PdpNode *node, int depth);

/*
 * Check whether username holds all bits in required_perm.
 * Returns CADMUS_OK, CADMUS_ERR_PERMISSION_DENIED, or CADMUS_ERR_NOT_FOUND.
 */
